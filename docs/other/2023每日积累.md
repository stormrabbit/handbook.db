...
function getCookie(name, domain) {
  ...
  for (let i = 0; i < cookies.length; i++) {
    if (cookies[i].match(regex)) {
      const cookieDomain = cookies[i].split(';')[1]?.trim().split('=')[1];
...
const token = getCookie('token', 'example.com');
console.log(token);

```

这个函数接受两个参数：要获取的 cookie 名称和指定的域名。首先，它将 document.cookie 按 "; " 分割成一个字符串数组，每个元素代表一个 cookie。然后，它使用正则表达式匹配出名为 name 的 cookie，并检查它的域名是否与指定的域名匹配。如果匹配成功，就返回该 cookie 的值；否则返回 null。

需要注意的是，如果 cookie 的域名属性设置为顶级域名（例如 .example.com），那么可以使用该顶级域名来匹配所有子域名下的 cookie。如果没有指定域名，则默认为当前页面的域名。

### 2. 为什么用这个方法获取到的 cookie 是 null 呢？

有几种可能会导致通过这种方式获取 cookie 返回 null：

指定的域名不正确：获取 cookie 的域名应该与设置 cookie 时指定的域名完全匹配（包括子域名）。如果指定的域名不正确，就无法匹配到相应的 cookie。

cookie 被设置为 HttpOnly：如果 cookie 的 HttpOnly 属性设置为 true，那么它将无法通过 JavaScript 访问，这是为了防止跨站点脚本攻击（XSS）。

cookie 已过期或被删除：如果 cookie 的过期时间早于当前时间，或者它已经被删除，那么它就无法获取到了。

可以通过浏览器的开发者工具来检查指定域名下的 cookie 是否存在，并查看它的属性设置。同时，也可以尝试使用其他方法来获取 cookie，比如使用浏览器提供的 API（如 document.cookie 或 CookieStore）或第三方库（如 js-cookie）。

### 3. 如何查看 cookie 是否被设置为 HttpOnly ？

要查看 cookie 是否被设置为 HttpOnly，可以按照以下步骤：

打开浏览器的开发者工具，切换到 Network 面板。
访问网站，使得网站的响应出现在 Network 面板中。
选中该请求，然后查看 Response Headers 部分。
在 Response Headers 中找到 Set-Cookie 字段，并查看它的属性值。
如果 cookie 被设置为 HttpOnly，则 Set-Cookie 字段中会包含 HttpOnly 属性。例如：

```
Set-Cookie: session_id=12345; path=/; domain=example.com; HttpOnly
```

如果 cookie 没有被设置为 HttpOnly，则 Set-Cookie 字段中不会包含 HttpOnly 属性。
